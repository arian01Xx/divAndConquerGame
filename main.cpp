...
public:
    int kthLargestValue(vector<vector<int>>& matrix, int k) {
        ...
        for(int i=0; i<m; i++){
            for(int j=0; j<n; j++){
                if(i==0 && j==0) xorMatrix[i][j]=matrix[i][j];
                else if(i==0) xorMatrix[i][j]=xorMatrix[i][j-1]^matrix[i][j];
                else if(j==0) xorMatrix[i][j]=xorMatrix[i-1][j]^matrix[i][j];
                else{
                    xorMatrix[i][j]=xorMatrix[i-1][j]^xorMatrix[i][j-1]^
                                    xorMatrix[i-1][j-1]^matrix[i][j];
                }
